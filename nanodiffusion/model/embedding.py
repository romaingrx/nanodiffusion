...
class TokenEmbedding(eqx.Module):
    ...
    def __init__(self, vocab_size: int, embed_dim: int, *, key: PRNGKeyArray) -> None:
        self.embed = eqx.nn.Embedding(vocab_size, embed_dim, key=key)
        weight = jax.random.normal(key, (vocab_size, embed_dim)) * 0.02
...
class TimeEmbedding(eqx.Module):
    ...
    def __init__(self, dim: int, *, key: PRNGKeyArray) -> None:
        ...
        self.linear_1 = eqx.nn.Linear(dim, dim, use_bias=True, key=split())
        self.linear_2 = eqx.nn.Linear(dim, dim, use_bias=True, key=split())
